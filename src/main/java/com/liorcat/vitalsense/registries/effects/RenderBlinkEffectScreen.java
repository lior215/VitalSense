...
public class RenderBlinkEffectScreen {
    ...
    public static void render(double screenDivider, int multiplier, boolean renderBelow, float transparency) {
        ...
        if (BlinkEvents.getPlayerBlinking()) {
            pose.pushPose();
    ...
    public static class RenderBlinkEventClass {
        ...
        public static void onRenderTick(TickEvent.RenderTickEvent event) throws NoSuchFieldException, IllegalAccessException {
            if (event.type == TickEvent.Type.RENDER && event.side.isClient() && event.phase == TickEvent.Phase.END && BlinkEvents.getPlayerBlinking() && ModCommonConfigs.ToggleBlinkMechanic.get()) {
                ...
                if (Timer.getTimer() <= 0) {
                    ...
                    BlinkEvents.setPlayerBlinking(false);
                    BlinkEvents.setCanStartBlinkingTimer(true);
                }
                ...
                if (BlinkEvents.getPlayerBlinking() && Minecraft.getInstance().options.hideGui && ModCommonConfigs.ToggleBlinkRenderOnF1.get()) {
        ...
        public static void playerBlink(RenderGuiOverlayEvent.Post event) {
            if (event.getOverlay().id() == VanillaGuiOverlay.VIGNETTE.id() && BlinkEvents.getPlayerBlinking() && ModCommonConfigs.ToggleBlinkMechanic.get()) {
                render(screenDivider, multiplier, false, 1.0f);
